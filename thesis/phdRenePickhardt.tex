...
\section{Formal Definition and Construction of Language Models and $n$-gram models}
...
\end{align}
a family of conditioned \todo{omit conditioned in the name?} unigram models of order $n$ if $\forall h\in W^n$ the associated function $P_1^n(\cdot ; h):S\longrightarrow [0,1]$ is a unigram model over $W$. \todo{make sure index shifts are sensefull. it could also be $n-1$ or $n+1$}
\end{definition}
\begin{remark}
\begin{description}
...
\begin{align}
P_1^{n-1}:(w;w_2\dots w_n):=\frac{1}{|W|}\sum{v\in W}P(w;v w_2\dots w_n)$
\end{align}
...
\item the $n$ -gram  model \todo{stupid name! need a better one for our own method}
\end{itemize}
