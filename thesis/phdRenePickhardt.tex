...
In the case of finite $\Omega$ --- which is almost always our case --- the $\sigma$-algebra has to be the power set $\pow{\Omega}$ \cite{FINDMATHBOOK}.
...
Let $w_1^n:=w_1\dots w_n$ be a sequence of $n$ words and $c(w_1^n)$ the frequency of the sequence in a given training corpus we can define $P(w_1^n):=\frac{c(w_1^n)}{N_n}$ where $N_n:=\sum_{w_1^n}c(w_1^n)$.
...
One way to work this out\footnote{one could also try to create a real language model on $\Omega=W^*$ which --- starting from the formula --- would be rather complex but possible to sum to $1$. The important point to note is that already with the most basic definition we see that there is some choice how we interpret the formula.} is to say that the above formula defined not one probability function but for every $n$ it defined a different probability function $P_n:\pow{W^n}\to[0,1]$.
Note that for each $P_n$ we had to define a different $\sigma$-algebra coming from a different $\Omega=W^n$. 
Explicitely this means that for every $n$ we will have $P_n(\{(w_1,\dots,w_n)\}):=\frac{c_n((w_1,\dots,w_n))}{N_n}$ where $c_n:W^n\to IN$ is the counting function saying how often the $n$-gram occured in the training data and $N_n$ is defined as above.
The definition only defined $P_n$ on $\Sigma_1=\{A\in\Sigma|$ $|A| = 1\}\subset \Sigma$ which are the events of $\Sigma$ that contain only one element.
Due to the fact that $\Omega$ is finite all other elements of $\Sigma$ can be created by finite units. 
By the axioms of a probability function we must have $P_n(A_1 \union \ldots \union A_j) = P_n(A_1) + \ldots + P_n(A_j)$.
Thus with the above interpretation of the formula $P_n$ is well defined on all of $\Sigma$.
\todo{not clear of the following should be mentioned.}In particular we implicitly used a projection function $\pi:\Sigma_1\to W^n$  which maps the elements of $\Sigma$ which as subsets of $\Omega$ contained only one element to this particular element.
...
In particular this would mean that $P$ is just defined on $\pow{W}$ which is most certainly not what the computer scientist meant \todo{spelling}to say.
...
\textbf{Definition:} A $n$-gram language model or just in short $n$-gram model is a probability function $P_n:\pow{W_n}\to [0,1]$ where $W$ is a \textbf{finite} set of Words in natural language. 
Obviously fixing ourselves to the case of words is an arbitrary choice.
