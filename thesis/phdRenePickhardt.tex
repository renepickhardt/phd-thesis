...
\begin{example}\label{ex:unigram-lm}
...
We can use this to construct an 'unigram'  language model $(W^{*},2^{W^{*}},P)$. 
 
Before we doe so we first realize that the sets containing just $s$ with $s = w_1\dots w_m \in W^{*}$ will generate the $\sigma$-algebra $S = 2^{W^{*}}$. 
By \ref{ex:continuation} it is enough to construct the language model on these elements of $S$.  

Before we show the correct construction let us first show a construction that looks promising but will not work. We do this to illustrate the pitfalls one can run into.

We set $P'(\{s\}) = \prod_{w\in s}P_1(\{w\})$ \todo{technically $w\in s$ the in-operator is not well defined!}

$P'$ is not a probability distribution because $P'(\bigcup_{w\in W}\{w\})= P_1(\bigcup_{w\in W}\{w\})=\sum_{w\in W}P_1(\{w\}) = 1 \Rightarrow P'(W) = 1$. 
On the other hand if $P'$ were a probability distribution we would have $P'(W^{*}) =1$ by definition. 
If both was true than due to $\sigma$-additivity for every $s= \{v,w\}$ we would have $P'(\{s\})=P_1(\{v\})P_1(\{w\}) = 0 \forall v,w \in W$.
This can only be true if $P_1(\{w\}) = 0 \forall w$.
That is an obvious contradiction to the assumption that $P_1$ is a probability distribution. 
Therefore we conclude that $P'$ could not be a probability distribution or a language model. 

The correct construction makes use of the parameter $\rho$ by setting $P(\{s\}) = \rho\prod_{w\in s}(1-\rho)P_1(\{w\}) =  \rho(1-rho)^{m}\prod_{w\in s}P_1(\{w\})$
We can already see that $P(W)=\rho(1-\rho) < 1$ leaving probability mass for longer sequences. 
In a similar fashion we see $P(W^2)=\rho(1-\rho)^2$ and obviously $P(W^m)=\rho(1-\rho)^m$. 
Now $P(W^{*}) = P(\bigcup_{i=1}^\infty W^i) = \sum_{i=1}^infty P(W^i) = \sum_{i=1}^\infty \rho(1-\rho)^i = \roh \sum_{i=1}^\infty (1-\rho)^i = \rho \frac{1}{1-(1-\rho)}  = \fraq{\rho}{\rho} = 1$. 
We could only use the convergence formula $\sum{n=0}^infty q = \frac{1}{1-q}$ of the geometric series since $\rho$ was a value between $0$ and $1$. \todo{I need to fix the proof since in my case the sum starts with $1$ and not with $0$. This needs to fixed in the model but it is one of the typical border cases and will be easy to fix. Also the way how I encoded the length distribution as a geometric distribution is consistent with what is being done in the literature but it is not reflecting the real sentence length distributions.}
 
\end{example}

\begin{remark}
the parameter $\rho$ chosen in example \ref{ex:unigram-lm} can be seen as the probability of a sentence to end and $1-rho$ can be seen as the probability of a sentence to continue. 
With the help of this trick we did not have to extend our probability space with an begin of sentence or an end of sentence token. Although when writing software this might be very convenient from a mathematical point of view this is highly questionable because summing up to $1$ now goes over the subsets that have exactly one begin of sentence token in the beginning and and end of sentence token at the end of a sentence. 
\end{remark}
