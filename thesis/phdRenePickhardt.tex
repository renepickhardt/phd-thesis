...
\todo{add glossary terms: language model, n-gram Model, language model of order n, number of model parameters}
In this chapter we will give an overview of the methods in statistical $n$-gram based language modeling and provide a mathematically rigorous and sound notation for language models.
...
Given a finite Vocabulary $\Omega = \{w_1,\dots,w_N\}$ of $N$ words. 
\begin{definition}
$S$ is called the $\sigma$-algebra over $\Omega$ iff:

\begin{enumerate}
\item $S\subset 2^{\Omega}$ where $2^{\Omega}$ is the power set of $\Omega$  \eqnum
\item $A_1,A_2,\dots \in S \Rightarrow \bigcup_{n\in\mathbb{N}}A_n \in S$ \eqlab{eq:closedUnion}
...
Due to \eqref{eq:closedUnion} it is sufficient to know $P$ on events containing just one element of $\Omega^{*}$ if these events generate $S$. 
Knowing $P$ on all events which contain just one element of $\Omega^{*}$ is particularly useful since these events correspond to sequences of words in $\Omega$. 

\todo{emphasize somehow}The main question throughout this thesis is: 
...
\begin{definition}
The subsets $\{(w_1,\dots,w_n)\}\subset S$ of events containing just one element will be named $w_1^n$  
\end{definition}
