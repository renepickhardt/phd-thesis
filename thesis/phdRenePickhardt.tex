...
In the case of finite $\Omega$ the $\sigma$-algebra will always be the power set $\pow{\Omega}$. 
...
Since $\Omega$ is finite $\Sigma=\pow{\Omega}$ will most of the time not be named explicitly.  

Let us now consider the computer scientist point of view together with a first example:
Let $w_1^n:=w_1\dots w_n$ be a sequence of $n$ words and $c(w_1^n)$ the frequency of the sequence in a given training corpus we can define the maximum likelihood probability $P(w_1^n):=\frac{c(w_1^n)}{N_n}$ where $N_n:=\sum_{w_1^n}c(w_1^n)$.
While this definition seems to be clear and is much shorter than all the technical details needed for mathematicians formulation we will quickly see that there is much trouble with definitions like this.
Let us first try to see the problems a mathematician has with the above defition. \todo{they might seem negligable but in fact they are not as mentioned above}
It is not clear if $P$ is a probability function and what its probability space is.
The best way to work this out will actually be to say that the above formula defined an infinite amount of probability functions $P_n$ for every $n \in IN$. 
Each $P_n$ is defined defined on different $\sigma$-algebra coming from a different $\Omega$. 
Explicitely this means that for every $n$ we will have $P_n:\pow{W^n}\to [0,1]$ with $P_n(\{(w_1,\dots,w_n)\}):=\frac{c_n((w_1,\dots,w_n))}{N_n}$ where $c_n:W^n\to IN$ is the counting function and $N_n$ is defined as above.
\todo{there is another subtlety here with the counting function with now goes from omega but P goes from the sigma algebra. Even worse only du to the axioms of probability we have defined $P$ on complete $\Sigma$. We can see it is a lot of overhead in notation to be precise}
